...
Vagrant.configure("2") do |config|
  ...
  config.vm.box = "ubuntu/focal64"
   ...
   # Определяем конфигурацию для первой ВМ
  config.vm.define "vm1" do |vm1_config|
    ...
    vm1_config.vm.provision "shell", inline: <<-SHELL
      ...
      echo "htop installation finished."
    SHELL

    vm1_config.vm.provider "virtualbox" do |vb|
      vb.name = "Ubuntu VM1 (Vagrant Group)"
    end
  end

  # Определяем конфигурацию для второй ВМ
  config.vm.define "vm2" do |vm2_config|
    vm2_config.vm.hostname = "ubuntu-vm2"
    vm2_config.vm.network "private_network", ip: "192.168.56.102"

    # Синхронизируем локальную папку с /home/vagrant/shared на ВМ
    vm2_config.vm.synced_folder "./sharedData", "/home/vagrant/shared", type: "rsync"
    ...
    vm2_config.vm.provider "virtualbox" do |vb|
      vb.name = "Ubuntu VM2 (Vagrant Group)"
    end
  end
